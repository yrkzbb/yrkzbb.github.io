...
### [](#1-NoSQL-和-SQL-的区别是什么？)1\. NoSQL 和 SQL 的区别是什么？
...
### [](#1-INT-1-和-INT-10-在-MySQL-中有什么不同？)1\. `INT(1)` 和 `INT(10)` 在 MySQL 中有什么不同？
...
### [](#2-TEXT-数据类型可以无限大吗？)2\. `TEXT` 数据类型可以无限大吗？
...
### [](#3-IP-地址如何在数据库中存储？)3\. IP 地址如何在数据库中存储？
...
### [](#2-MySQL-中-IN-和-EXISTS-有什么区别？)2\. MySQL 中 `IN` 和 `EXISTS` 有什么区别？
...
### [](#1-MySQL-中的一些基本函数，你知道哪些？)1\. MySQL 中的一些基本函数，你知道哪些？
...
### [](#1-给定学生成绩表-student-score-stu-id-subject-id-score-，查询总分排名第-5～10-名的学生-ID-及对应的总分)1\. 给定学生成绩表 `student_score(stu_id, subject_id, score)`，查询总分排名第 5～10 名的学生 ID 及对应的总分
...
### [](#1-如何用-MySQL-实现一个可重入锁？)1\. 如何用 MySQL 实现一个可重入锁？
...
### [](#1-MySQL-如何避免重复插入数据？)1\. MySQL 如何避免重复插入数据？

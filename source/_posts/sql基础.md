...
`IN` 用于判断某个值是否存在于列表或子查询结果集中，写法直观，适合子查询结果集较小的场景。

`EXISTS` 只判断子查询是否至少能返回一行数据，它不关心子查询返回什么数据，只关心是否有结果。如果子查询有结果，则`EXISTS` 返回`TRUE`，否则返回`FALSE`。它通常用于关联子查询，找到第一条匹配记录后就可以停止查找，适合外层结果集较小、子查询表较大且关联字段有索引的场景。
...
`IN` 比较的是具体值，通常先得到子查询的结果集再进行匹配；`EXISTS` 判断的是记录是否存在，通常会针对外层查询的记录执行关联判断，命中一条即可返回。

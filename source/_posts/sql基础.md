...
`IN` 用于判断某个值是否属于列表或子查询结果集；`EXISTS` 用于判断子查询是否至少返回一行。

它们的主要区别是：

- **语义差异**：`IN` 比较具体值是否属于结果集；`EXISTS` 只判断是否存在满足条件的记录。相关子查询中，`EXISTS` 找到匹配行后即可停止继续查找。
- **使用场景**：表达“某个值是否在结果集合中”时，`IN` 通常更直观；表达“是否存在满足关联条件的记录”时，`EXISTS` 通常更符合语义。
- **性能差异**：不能简单认为 `EXISTS` 一定比 `IN` 快。现代 MySQL 优化器可能将两者转换为半连接，并根据数据量、索引和统计信息选择执行策略，应结合 `EXPLAIN` 或 `EXPLAIN ANALYZE` 判断。
- **NULL 处理**：`IN` 和 `NOT IN` 遵循三值逻辑。特别是子查询结果包含 `NULL` 时，`NOT IN` 可能得到 `UNKNOWN`，从而返回非预期结果。`EXISTS` 和 `NOT EXISTS` 只判断是否存在满足条件的行，因此处理反向匹配时通常更安全。

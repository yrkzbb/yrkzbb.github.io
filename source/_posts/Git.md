...
我能够熟练使用 Git 完成日常开发和团队协作，包括分支管理、提交整理、合并与变基、冲突解决、版本回退、标签管理和问题定位。

日常工作中，我会保持提交粒度清晰，提交前使用 `git status` 和 `git diff` 检查变更；通过功能分支配合 Pull Request 做代码评审；根据团队规范选择 `merge`、`rebase` 或 `squash` 整理历史。遇到误提交、线上回滚或历史问题时，我也能根据提交是否已经共享，选择 `restore`、`reset`、`revert`、`reflog`、`bisect` 等合适的方式处理，而不是机械使用 `reset --hard`。

### 2. Git 有哪几个工作区域？它们之间是怎么流转的？
...
`git pull` 会先获取远程提交，再按配置执行合并或变基。工作区和暂存区可以存在不同版本，因此提交前要分别检查 `git diff` 和 `git diff --staged`。

### 3. 什么是 `HEAD`？什么是游离 `HEAD`？
...
### 4. `git checkout`、`git switch`、`git restore` 有什么关系？
...
- `git checkout -- file`：传统的文件恢复写法。

`checkout` 仍然可用，但新脚本和日常操作中我更倾向使用 `switch` 与 `restore`，因为不容易混淆“切分支”和“覆盖文件”。恢复文件会丢弃相应修改，执行前应先确认差异。
...
解决冲突时不能只删除标记，还要理解两边修改的业务意图，并运行测试验证结果。如果发现合并方向错误或暂时不应处理，可以执行 `git merge --abort` 回到合并前。推送前还应使用 `git diff --check`、`git log --graph` 等命令检查结果。
...
普通的非快进合并会创建一个具有两个父提交的 merge commit，从而保留“某个功能分支在这里被合入”的边界。Fast-Forward 历史更线性，但看不出原来的分支边界。`--ff-only` 在不能快进时直接失败，适合禁止隐式合并提交的场景；`--no-ff` 则适合希望保留功能合并节点的团队。
...
- 普通 `pull` 在本地与远程分叉时可能产生 merge commit；
- `pull --rebase` 通常得到线性历史，但会改写本地提交的哈希；
- 如果只能接受快进，可以使用 `git pull --ff-only`，分叉时让命令失败并由开发者决定如何处理。

是否默认变基还会受到 `pull.rebase`、分支配置等影响，因此不能脱离配置断言 `git pull` 永远等于 `fetch + merge`。变基发生冲突时，解决后执行 `git add` 和 `git rebase --continue`，放弃则使用 `git rebase --abort`。
...
`cherry-pick` 会产生内容相似但哈希不同的新提交。大量使用可能造成重复提交和后续合并冲突，因此整条分支的常规整合通常更适合使用 `merge` 或 `rebase`。
...
`git fetch` 只下载远程仓库的新对象并更新远程跟踪分支，不会自动修改当前分支和工作区；`git pull` 会先获取，再把远程变化整合进当前分支。
...
因此需要先审查远程变化时，我更倾向使用 `fetch`，确认后再手动 `merge` 或 `rebase`。`pull` 更快捷，但工作区不干净、分支已分叉或整合策略不明确时，可能产生冲突或意外的历史形态。

### 2. 常见的 Git 工作流有哪些？

常见工作流主要有以下几类，没有一种适合所有团队：

- **集中式工作流**：成员围绕单一主分支协作，流程简单，适合小型团队，但需要严格控制直接推送。
- **Feature Branch / GitHub Flow**：每个需求建立短期功能分支，通过 Pull Request 评审后合入主分支，适合持续交付。
- **Git Flow**：使用 `main`、`develop`、`feature`、`release`、`hotfix` 等长期或短期分支，适合发布周期明确、需要维护多个版本的项目，但流程较重。
- **Trunk-Based Development**：开发者频繁把小批量变更集成到主干，通常配合自动化测试、功能开关和短生命周期分支，适合高频交付。
- **Forking Workflow**：贡献者在自己的 Fork 中开发，再向上游提交 Pull Request，常用于开源项目或权限隔离较强的协作。

选择时要结合团队规模、发布频率、自动化测试能力、代码评审要求和多版本维护成本，而不是只追求某种固定分支模型。

### 3. Fork、Clone、Pull Request 在 GitHub 协作中分别是什么？

三者处于不同层次：

- **Fork**：在 GitHub 服务端把他人的仓库复制到自己的账号或组织下，形成独立仓库，常用于没有上游写权限的协作。

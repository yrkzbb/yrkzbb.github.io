...
const SERIES = [
  ...
  },
  {
    key: "network",
    name: "计算机网络",
    titles: ["网络模型", "应用层", "运输层", "网络IO", "网络场景", "网络攻击"],
  },
  {
    key: "os",
    name: "操作系统",
    titles: ["用户态和内核态", "进程管理", "中断", "内存管理", "锁"],
  },
];

function names(collection) {
  return collection && collection.toArray
    ? collection.toArray().map((item) => item.name)
    : [];
}
...
hexo.extend.helper.register("related_posts", function (post, limit = 5) {
  ...
  return this.site.posts
    ...
    .map((item) => {
      const score =
        names(item.tags).filter((tag) => postTags.has(tag)).length * 3 +
        names(item.categories).filter((category) =>
          postCategories.has(category),
        ).length;
      return { item, score };
    })
...
hexo.extend.generator.register("knowledge_outputs", function (locals) {
  ...
  const manifest = posts.map((post) => ({
    ...
    excerpt: stripHtml(post.description || post.excerpt || post.content).slice(
      0,
      140,
    ),
  }));
  ...
  for (let offset = 6; offset >= 0; offset -= 1)
    updates[today.clone().subtract(offset, "days").format("YYYY-MM-DD")] = 0;
  let words = 0;
  posts.forEach((post) => {
    const text = stripHtml(post.content || "").replace(/\s+/g, "");
    words += text.length;
    names(post.categories).forEach((name) => {
    ...
    if (Object.prototype.hasOwnProperty.call(updates, updateDay))
      updates[updateDay] += 1;
  });
  ...
  const items = posts
    .map((post) => {
      ...
      return `<item><title>${escapeXml(post.title)}</title><link>${escapeXml(url)}</link><guid isPermaLink="true">${escapeXml(url)}</guid><pubDate>${post.date.toDate().toUTCString()}</pubDate>${names(
        post.categories,
      )
        .concat(names(post.tags))
        .map((name) => `<category>${escapeXml(name)}</category>`)
        .join(
          "",
        )}<description>${cdata(stripHtml(post.description || post.excerpt || post.content).slice(0, 300))}</description><content:encoded>${cdata(post.content)}</content:encoded>${image ? `<media:content url="${escapeXml(absolute(image))}" medium="image"/>` : ""}</item>`;
    })
    .join("");
